...
3. We defined rules to describe how populations move and engage in conflict as they expand in Chapter&nbsp;\@ref(conflict).
4. We simulated these process of growth, conflict and migration on the environement we designed, for extended periods of time in Chapter&nbsp;\@ref(main-simu).
...
Here we will use all these previous steps to generate csv files that can be shared. In this chapter we will use the simulation run in the Chapter&nbsp;\@ref(main-simu) ; if you want to generate a new 'fake'-dataset, different from the one that have been presented in the original challenge and that which will be described in the chapter&nbsp;\@ref(original-challenge), you will want to run longer simulations, and choose from the  different runs one that looks interesting/realistic. We provide a script `./scriptmini.R`  to give some ideas about how to automtate these runs and how to make a choice.



## Using sites size to generate deposit

This has been done using the file `dateGeneration.R`. We will describe it here now.

First we get the rasters defining the environment (If you have followed the book from the beginning they should be already in memory but in case you didn't we we reload them here):
...
Using `Rec_c` , `short_loss` and `long_loss`, that we presented in Chapter&nbsp;\@ref(recgen) &&nbsp;\@ref(recloss), we generate deposit. This can take a long time if the simulation included a lot of sites, spanning a long period of time. Here we first generate all deposit from the matrix `Nts` which was generated during the simulation of the Chapter&nbsp;\@ref(main-simu) and then apply all loss functions together:
...
Once this is done, we have a list with all the potential dates available from all sites that have existed during our simulation. Realistically, for any given site, not _all_ of the material will be recovered, and even less of this material will be dated. To reflect that, we capped the maximum number of samples available per site. To do so -- and this was applied as is in the original challenge presented in the Chapter&nbsp;\@ref(original-challenge), we assume that the biggest sites -- the one with most media attention, most funding, will very likely get at most 30 dates. We thus subsampled the number of dates for smaller sites given this ration of $\frac{30}{n_{\text{largest}}}$. Here simulations have been greatly shortened, thus sites have way fewer samples. We thus cap the large sites to have only 80% of the dates it generates, and sample the other sites using this ratio.
...
We then have a sample of dates for every site of our worlds; where the maximum number of dates is 30, for the biggest sites that existed in our simulation. 

Let's look at where these dates are available: We first retrieve all sites saved in the simulation from&nbsp;\@ref(main-simu):
...
Remove BP from the dates, and add some random variation:
...
To then share these dates we choose to split the environment in to grids and extract dates by grid. For each grid, a csv is generated with the sites that have been found and for each site, their geolocalisation and the dates found at the site.
...
In the archaoriddle original challenge we proposed to give the data from 4 grids that we selected ourselves to be sure the participants have at least _some_  interesting information. Here we randomly pick up 4 squares with stipulation that they provide at least 5 settlements. You will want to change and manually populate the vector `selection` in the chunk below to cover area with lot of sites and sites from different cultures, from what can be seen in the Figure&nbsp;\@ref(fig:plotallfoundiste-ex).
...
### Giving names to the publicly available data

On the original challenge we used: [fantasynamegenerators.com](https://www.fantasynamegenerators.com/fantasy-town-names.php) to choose nice, randomly generated name, to name the sites publicly available. You will see in the section&nbsp;\@ref(naming) that we positionned them with cultural relevent group, although this has no impact on the results. Here we took all the names we had for the original challenge to randomly assign some to your custom world.
...
The csv files generated for this specific run of the bookdown can be downloaded for each square:

```{r create-links,echo=FALSE,results="asis"}
for (g in selection) cat(paste0("- [square ",g,"](data_toshare/square_",g,".csv)\r\n"))
```

Export csv and data for all squares:
...
The csv files generated for all squares can be [downloaded here](data_tmp/)
