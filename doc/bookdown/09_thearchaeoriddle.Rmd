...
In this chapter we willgo through all the steps we presented in Chapters&nbsp;\@ref(env) to&nbsp;\@ref(final-output), but in a condensed way and with the parameters used to generate the data shared during the Archaeoriddle original challenge. But before that we will we retrace the journey from the first discussions in the courtyard of the McDonald Institute in Cambridge to the final workshop in Belfast.

We will then end the chapter by discussing briefly the proposal submitted for the challenge.

## Context 

Here is a bit about how all that happened, and the major steps.
...
After discussion within the membership of the CDAL, we decided that a challenge-like event, with a cash-prize, would be the best way to convince people to participate. After the British Academy grant was awarded to Simon Carrignon, Alfredo Cortell and Enrico Crema, we decided to announce this challenge through various social media videos and a website: https://theia.arch.cam.ac.uk/archaeoriddle/
...
In addition to allowing participants to download the public dataset, and to select new squares to survey, the website mentioned in the call listed a series of research questions intended to guide participants in their endeavours.

#### Research Questions

- *RQ1.*  The Poppy-chewers and Rabbit-skinners had a hostile relationship.

- *RQ2.*  Poppy-chewers exhibited an exponential population growth, while Rabbit-Skinners eventually stalled, and as the Poppy-chewers' population size started to increase, their population declined (Fig. 3).
...
The website where these research question will disappear soon, so here is a snapshot of the first page:


But in case this disappears, the source to reproduce the website is also available in the github Archaeoriddle repository [here](https://github.com/acortell3/archaeoriddle/tree/main/doc/shinyapp).

The call was issued in April 2023, after acceptance of the workshop at EAA, and 5 proposals were submitted. 

During the actual workshop, the whole project was summarised, the 5 proposals described by their respective authors, and the exact simulation revealed.
...
But because [Simon Carrinon](sc.fram.io) did a poor job when he ran the simulation, he didn't generate seeds, nor store them, for each experiment; so we cannot re-run the original one _exactly_ as it happened. But he kept a lot of information about and output of it which, are stored in the folder `general_results_selected_simu/`
...
3. Run hundreds of simulations and choose the one which looked the coolest 😆.
...
One can see, from line 52 until the end of the `scriptmini.R`, that some graphics are saved. These will be used to quickly compare the different simulations and choose the good one.

The simulations are also run with the parameter: `visumin=TRUE `, which allows the saving of images of population growth and settlement creation at every time step while the model is running. These can then be used to generate video of the whole process using a simple script such as:
...
where `test*` is the folder storing all output of a simulation. We provide the simulations that were generated during this preliminary phase in the zenodo folder associated with the first paper published fomr the Archaeoriddle project in [this archive](https://zenodo.org/records/14024548/files/run_selection.tar.gz) (Warning: its 1.2GB).

Here are videos showing a selection of some of the scenarios discarded while choosing the ultimate run:
...
This should also correspond exactly to what is stored in `data_original/resources.tiff`
...
And this is `Rabbithole`,  home of the Poppy-chewer and Rabbit-skinner!
...
Using `Rec_c` , `short_loss` and `long_loss` we generated deposit from the records of the run selected from the original challenge.  This takes a long time when applied for all sites for all the time of simulations but can be done:
...
rm(alldeposit) # We suggest you remove the deposit object, which can be large and isn't used afterwards
```

We then limited the amount of sample available, with the assumption that if the biggest site was to be excavated as much as possible, we will not have more than 30 dates from it. Thus we randomly sampled after having normalised by this max amount:
...
Although the seed used for the sampling hasn't be saved, the output of these steps has been saved and can be loaded back:
...
We then make it a bit annoying by removing BP from the dates, and we add a useless and meaningless different standard deviation:
...
Again, this is stochastic (the selection of the fake standard variation added is random), so the exact results can't be reproduced exactly, although in that case these steps don't change anything. The outcome of these steps is available in the folder [data_original/all_squares.oc/](./data_original/all_squares.oc/).

### Grid generation and sharing

Split the environment in to grids and extract dates for each grid:
...
### Giving names to the publicly available data {#naming}

For the original challange we wanted sites to have a name for the sites publicly available. To do so we used: [fantasynamegenerators.com](https://www.fantasynamegenerators.com/fantasy-town-names.php). We manually positioned to generate some culturally relevant group depending on their geographical position, in a way slightly inspired by the reality ;  although this has no impact on the results. 
...
We can then export `csv`s and data for each public square, and the non-public one. We here simply follow what was presented in Chapter&nbsp;\@ref(public-data):
...
P1 (https://github.com/dpriss/Archaeoriddle_Kahlenberg_Priss) was based on agent-based modelling (ABM) combined with exploratory data analysis. It first studied the land of Rabbithole and calibrated the dates provided, which were then used to compute trajectories of dispersal, and to study site preference using ArcGIS Pro and R. The suggested dispersal rates over land and water were close to the Archaeoriddle solution. The proposal then used the results of its analyses to fit the ABM, which was built using NetLogo. For the ABM, moving groups of hunter–gatherers and farmers, as well as the different settlements, were treated as agents, with starting values obtained from the exploratory data analysis and relevant literature. Additionally, it introduced behavioural rules, including movement and site preference, reaction to population threshold, and reaction to interaction. After running the model several times with different parameters, it correctly predicted a hostile relationship between the two groups and inferred an initial location of the Rabbit-skinners in the northeastern quadrant and the Poppy-chewers in the southwestern quadrant, while also detecting the latter's northward movement. The expansion rate was not captured by the initial assumption of logistic growth in the areas already densely inhabited by Poppy-chewers, which makes sense considering the different population trajectories of the two groups. 
